...
The original Waterbirds path in ``experiment_runner.py`` compares every
...
def extract_index_feature_cache(
    ...
    from experiment_runner import FeatureExtractor
...
def run_waterbirds_reference_experiment(
    waterbirds_root: str,
    train_size: int,
    test_size: int,
    batch_size: int,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/results/improved/results_waterbirds_improved"),
    )
    parser.add_argument("--random-weights", action="store_true")
    args = parser.parse_args()

    summary = run_waterbirds_reference_experiment(
