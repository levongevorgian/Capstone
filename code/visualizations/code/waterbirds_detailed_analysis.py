...
DEFAULT_RESULTS_DIR = Path("outputs/results/waterbirds/results_waterbirds")
DEFAULT_OUTPUT_DIR = Path("data/processed_data/waterbirds_analysis")


def parse_group_components(target_group: str) -> tuple[str | None, str | None]:
    """Extract bird label and background type from a Waterbirds subgroup name."""
...
def build_alignment_summary(df: pd.DataFrame) -> pd.DataFrame:
    """Summarize high-level matched-versus-mismatched behavior."""
...
def build_subgroup_summary(df: pd.DataFrame) -> pd.DataFrame:
    """Return subgroup-level rows with parsed label/background columns."""
    subgroup_rows = df[df["target_group"].str.contains("landbird_|waterbird_", regex=True)].copy()
...
def build_environment_gap_summary(subgroup_df: pd.DataFrame) -> pd.DataFrame:
    """Quantify how much each label changes across land and water backgrounds."""
    rows = []
...
def build_worst_group_summary(subgroup_df: pd.DataFrame) -> pd.DataFrame:
    """Summarize best and worst subgroup performance for each split/model pair."""
    rows = []
...
def write_report(
    ...
    """Write a concise Waterbirds interpretation report in Markdown."""
    lines: list[str] = []
...
def main() -> None:
    ...
    write_report(
        args.output_dir / "waterbirds_detailed_analysis.md",
        corr_df,
        alignment_summary,
        environment_gaps,
        worst_groups,
