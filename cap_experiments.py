...
def load_cifar_dataset(name: str, train: bool, allow_download: bool):
    ...
    try:
        return dataset_cls(root="./data", train=train, download=False)
    except RuntimeError:
        if allow_download:
            return dataset_cls(root="./data", train=train, download=True)
        split = "train" if train else "test"
        raise RuntimeError(
            f"{name} {split} split is not available locally in ./data and downloads are disabled. "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bloodmnist-root", type=str, default="./data", help="Root directory for BloodMNIST cache/download.")
    parser.add_argument("--waterbirds-root", type=str, default=None, help="Root directory for Waterbirds dataset with metadata.csv.")
